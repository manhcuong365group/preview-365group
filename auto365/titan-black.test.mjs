...
assert.doesNotMatch(page, /id="tb-actual-2026"/, 'does not add a new real-image section when the request is layout-only');
assert.match(page, /const pageFlow = \[[\s\S]*?"kiem-tra-xe"[\s\S]*?"video-teaser"[\s\S]*?"nhu-cau"[\s\S]*?"evidence"[\s\S]*?"anh-sang"[\s\S]*?"thiet-ke"[\s\S]*?"so-sanh"[\s\S]*?"thong-so"[\s\S]*?"tuong-thich"[\s\S]*?"tb-why-buy"[\s\S]*?"tb-consult-cta"[\s\S]*?"case-xe"[\s\S]*?"lap-dat"[\s\S]*?"tb-trusted-address"[\s\S]*?"faq"[\s\S]*?"tb-final-cta"/, 'reorders the existing sections without creating new content blocks');
...
assert.doesNotMatch(page, /#tb-product #(evidence|nhu-cau)\{display:none\}/, 'keeps the approved needs and highlighted-specifications sections visible');
