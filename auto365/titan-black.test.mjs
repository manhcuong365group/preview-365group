...
assert.match(page, /const pageFlow = \[[\s\S]*?"kiem-tra-xe"[\s\S]*?"video-teaser"[\s\S]*?"nhu-cau"[\s\S]*?"evidence"[\s\S]*?"anh-sang"[\s\S]*?"thiet-ke"[\s\S]*?"thong-so"[\s\S]*?"so-sanh"[\s\S]*?"tuong-thich"[\s\S]*?"tb-why-buy"[\s\S]*?"tb-actual-2026"[\s\S]*?"tb-consult-cta"[\s\S]*?"case-xe"[\s\S]*?"lap-dat"[\s\S]*?"tb-trusted-address"[\s\S]*?"faq"[\s\S]*?"tb-final-cta"/, 'puts the Titan Black 2026 technical specifications before the three-generation comparison');
