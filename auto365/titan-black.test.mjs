...
assert.match(page, /const pageFlow = \[[\s\S]*?"kiem-tra-xe"[\s\S]*?"video-teaser"[\s\S]*?"nhu-cau"[\s\S]*?"thong-so"[\s\S]*?"anh-sang"[\s\S]*?"thiet-ke"[\s\S]*?"so-sanh"[\s\S]*?"tb-why-buy"[\s\S]*?"tuong-thich"[\s\S]*?"tb-fit-summary"[\s\S]*?"tb-actual-2026"[\s\S]*?"tb-consult-cta"[\s\S]*?"case-xe"[\s\S]*?"lap-dat"[\s\S]*?"tb-trusted-address"[\s\S]*?"faq"[\s\S]*?"tb-final-cta"/, 'uses the approved landing-page order, with full specifications at position 05 and no extra evidence section');
...
assert.match(page, /id="nhu-cau"[\s\S]*?<h2 style="margin-top:8px">Chọn Titan Black 2026 theo nhu cầu<\/h2>/, 'labels position 04 as the needs-selection section');
assert.match(page, /id="tb-why-buy"[\s\S]*?<h2 style="margin-top:8px">Titan Black 2026 giải quyết vấn đề gì\?<\/h2>/, 'labels position 09 as the problem-solving section');
