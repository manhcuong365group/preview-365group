...
assert.match(page, /@media \(min-width:481px\)\{[\s\S]*?\.tb-faq__form-actions \{ grid-template-columns:repeat\(3,minmax\(0,1fr\)\); \}[\s\S]*?\.tb-faq__form-actions \.tb-btn:nth-child\(3\) span\{display:inline\}/, 'balances all three contact actions while showing the nearby-address action on web');
...
assert.match(page, /class="tb-checker__grid">[\s\S]*?name="name"[\s\S]*?name="phone"[\s\S]*?name="area"[\s\S]*?name="brand"[\s\S]*?name="model"[\s\S]*?name="need"[\s\S]*?<\/div>\s*<div class="tb-checker__actions">/, 'keeps all six vehicle consultation fields in one grid for paired mobile rows');
