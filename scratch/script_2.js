<script>(()=>{
 ...
 function closeCompareModal() {
  ...
  const heroForm = $('#hero-sales-form');
 ...
 $('#mid-sales-form').addEventListener('submit', event => {
  ...
  sendAuto365LeadApi(apiPayload, leadId).then(res => {
    ...
    if (statusEl) {
      ...
      statusEl.innerHTML = 'Auto365 đã tiếp nhận yêu cầu. Mã lead: <strong>' + (res.lead_id || leadId) + '</strong>. Chuyên viên sẽ liên hệ lại ngay.';
    }
  ...
  emit('consult_submit', { name, phone, brand, model, province, need, leadId });
});
